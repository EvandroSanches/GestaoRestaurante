...
begin
  ...
  begin
    ShowMessage('Não há um registro selecionado para imprimir !');
    exit;
  end;
...
begin
     ...
     if Q_Pratos.IsEmpty then
     begin
          ShowMessage('Não há um prato selecionado para montar a composição !');
          exit;
     end;
...
begin
     ...
     begin
          ShowMessage('Não é possível excluir essa prato porque há composição cadastrada !');
          exit;
     end;
     ...
     if dm.Q_Geral.FieldByName('total').AsInteger > 0 then
     begin
          ShowMessage('Não é possível excluir essa prato porque há encomendas relacionadas !');
          exit;
     end;
...
// OnCreate do formulário f_pratos
